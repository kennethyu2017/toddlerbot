...
from kbot.locomotion.kbot_both_leg.joystick import try_g1_constants as consts
...
class TryG1Joystick(g1_base.G1Env):
  ...
  def _post_init(self) -> None:
    ...
    knee_indices = []
    knee_joint_names = ["knee_pitch_RS04"]
    for side in ["left", "right"]:
    ...
    for site in consts.FEET_SITES:
      sensor_id = self._mj_model.sensor(f"global_linvel_{site}").id
    ...
    self._feet_floor_found_sensor = [
        self._mj_model.sensor(f"floor_{foot_geom}_found").id
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    rng, key = jax.random.split(rng)
    qpos = qpos.at[7:].set(
        qpos[7:] * jax.random.uniform(key, (self._mjx_model.nu,), minval=0.5, maxval=1.5)
    )
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    # kenneth: temply debug.
    # done = done.astype(reward.dtype)
    done = done.astype(dtype=bool)
...
if __name__ == "__main__":
    ...
    test_env = TryG1Joystick(task=test_task)
    rng=jax.random.key(0)
    reset_state = test_env.reset(rng)
    print(f'{reset_state.data.qpos=:}')
    print(f'{reset_state.data.qvel=:}')
    print(f'{reset_state.data.qacc=:}')
    print(f'{reset_state.data.actuator_force=:}')
