...
class JoystickReward:
	...
	def get_rewards(
		...
		return {
			...
			"pose": self._cost_pose(data,init_q, cost_pose_weight),
		}
	...
	def _cost_contact_force(self, data: mjx.Data) -> jax.Array:
		...
		# for kbot-both-leg, without torso,should use 500 // 2
		cost = jp.clip(
	...
	def _cost_pose(self, data: mjx.Data, init_q: jax.Array, weight:jax.Array) -> jax.Array:
		...
		return jp.sum(jp.square(data.qpos[7:] - init_q[7:]) * weight)
	...
	def _reward_tracking_lin_vel(
		...
		# TODO: try, rational to cmd norm.
		# lin_vel_error = jp.linalg.norm(command[:2] - local_vel[:2])
		lin_vel_error = jp.sum(jp.square(command[:2] - local_vel[:2]))
		rwd = jp.exp( -lin_vel_error / self._rwd_cfg.tracking_sigma )
		sigma2=0.4
		rwd *=  jp.exp(jp.linalg.norm(command[:2]) / sigma2)
		return	rwd
	...
	def _reward_tracking_ang_vel(
		...
		# rational to cmd norm.
		rwd = jp.exp( - ang_vel_error / self._rwd_cfg.tracking_sigma)
		sigma2 = 0.4
		rwd *= jp.exp(jp.abs(command[2]) / sigma2)
		return rwd
	...
	def _cost_feet_slip(
		...
		body_vel_xy = data.sensordata[self._sensor_adr.pelvis_global_linvel_sensor_adr][:2]
		cost = jp.sum(jp.linalg.norm(body_vel_xy, axis=-1) * floor_feet_contact)
		return cost
