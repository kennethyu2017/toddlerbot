...
def _save_opt_result(robot_name: str,
                     opt_params_dict: Dict[str, Dict[str, float]],
                     opt_values_dict: Dict[str, float],
                     exp_folder: Path ):

    dyn_config_file = Path("toddlerbot") / "descriptions" / robot_name / "sysID_dynamics.json"

    assert len(opt_params_dict) > 0 and len(opt_values_dict) > 0
    ...
    if dyn_config_file.exists():
        with open(dyn_config_file, "rt") as _f:
            ...
            # update config according to the optuna result.
    ...
    with open(dyn_config_file, "wt") as _f:
        # JSON not allowed comments.
        # _f.write('//NOTE: this file is auto-generated by sysID_opt.py, do not change it.')
...
def _evaluate(
    ...
    """
    logger.warning(f'--- start evaluation --->')
    ...
    if dyn_config_file.exists():
        ...
        logger.warning(f'--- load dynamics param --- >: \n{json.dumps(dyn_config)}')

    else:
        raise FileNotFoundError(f'can not find dynamics file: {dyn_config_file.resolve()}')
    ...
    for _jnt_name, _ep_list in sysID_jnt_ep_dict.items():
        ...
        # always create new Mujoco for each joint sysID evaluate.
        sim = MuJoCoSim(robot, fixed_base=True)
        ...
        joint_dyn = {
            _jnt_name: {
                "damping": dyn_config[_jnt_name]["damping"],
                "armature": dyn_config[_jnt_name]["armature"],
                "frictionloss": dyn_config[_jnt_name]["frictionloss"],
            }
        }
        sim.set_joint_dynamics(joint_dyn)
        ...
        if "sysID" in robot.name:
            ...
            sim.set_motor_dynamics(
                { motor_name[0]: dict(
                    tau_max= dyn_config[_jnt_name]["tau_max"],
                    q_dot_tau_max= dyn_config[_jnt_name]["q_dot_tau_max"],
                    q_dot_max= dyn_config[_jnt_name]["q_dot_max"] )
                }
            )
    ...
    plot_save_folder: Path = exp_folder / 'plot'
    if not plot_save_folder.exists():
        plot_save_folder.mkdir()

    plot_folder_str = plot_save_folder.resolve().__str__()
...
def _main(args: argparse.Namespace):
    ...
    assert args.n_jobs < os.cpu_count() - 2
    ...
    exp_name = f'{robot.name}_sysID_mujoco_optim'
    ...
    if not args.eval_only:
        logger.warning(f'eval-only is False, we start optimization --->')
        ...
        _save_opt_result(robot.name,opt_params_dict,opt_values_dict,exp_folder)
...
def _args_parsing() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        required=True,
        help="The number of parallel opt jobs.",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Eval the `robot_name`_dynamics.json only without opt.",
    )
...
if __name__ == "__main__":
    ...
    config_logging(root_logger_level=logging.WARNING, root_handler_level=logging.NOTSET,
                   ...
                   log_file=None,
                   module_logger_config={'tools': logging.WARNING,
                                         'main': logging.WARNING})
    # use root logger for __main__.
    logger = logging.getLogger('root')
    logger.warning('parsed args --->\n{}'.format('\n'.join(
